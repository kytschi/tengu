...
/**
 * Proton invoice parser.
 *
 * @package     Kytschi\Wako\Parsers\ProtonParser
...
class ProtonParser extends ControllerBase
{
    public function parse($lines, $model, $type = '')
    {
        if (!$type) {
            foreach ($lines as $line) {
                if (strpos(strtolower($line), strtolower('Proton AG')) !== false) {
                    $type = 'proton';
                    break;
                }
            }
    ...
    private function process($lines, $model)
    {
        ...
        foreach ($lines as $line) {
            if (empty($amount)) {
                if (substr(strtolower($line), 0, strlen('Total EUR')) == strtolower('Total EUR')) {
                    $amount = NumberHelper::fromCurrency(trim(str_replace('Total EUR', '', $line)));
                    continue;
                }
            }
            ...
            if (empty($issued_on)) {
                if (substr(strtolower($line), 0, strlen('Due Date')) == strtolower('Due Date')) {
                    $issued_on = DateHelper::sql(
                        trim(str_replace('Due Date', '', $line)),
                        false
                    );
                    continue;
                }
