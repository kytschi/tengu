...
/**
 * Ebuyer invoice parser.
 *
 * @package     Kytschi\Wako\Parsers\EbuyerParser
...
class EbuyerParser extends ControllerBase
{
    public function parse($lines, $model, $type = '')
    {
        if (!$type) {
            foreach ($lines as $line) {
                if (strpos(strtolower($line), strtolower('Ebuyer UK LTD')) !== false) {
                    $type = 'ebuyer';
                    break;
                }
            }
    ...
    private function process($lines, $model)
    {
        ...
        foreach ($lines as $line) {
            ...
            if (empty($amount)) {
                if (substr(strtolower($line), 0, strlen('Invoice Total')) == strtolower('Invoice Total')) {
                    $amount = NumberHelper::fromCurrency(trim(str_replace('Invoice Total', '', $line)));
                    continue;
                }
            }
            ...
            if (empty($sub_total)) {
                if (substr(strtolower($line), 0, strlen('Subtotal')) == strtolower('Subtotal')) {
                    $sub_total = NumberHelper::fromCurrency(trim(str_replace('Subtotal', '', $line)));
                    continue;
                }
            }

            if (substr(strtolower($line), 0, strlen('Invoice no')) == strtolower('Invoice no')) {
                $refs[] = trim(str_replace('Invoice no', '', $line));
                continue;
            }
